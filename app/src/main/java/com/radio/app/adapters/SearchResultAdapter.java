...
public class SearchResultAdapter extends RecyclerView.Adapter<SearchResultAdapter.ViewHolder> {
    ...
    public SearchResultAdapter(Context ctx, List<SearchResult> results, OnSearchResultClickListener listener) {
        this.ctx = ctx;
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(ctx).inflate(R.layout.item_search_result, parent, false));
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder h, int pos) {
        ...
        h.tvType.setText(type != null ? type : "节目");
        h.tvTitle.setText(r.getTitle() != null ? r.getTitle() : "");
        h.tvStation.setText(r.getStationName() != null ? r.getStationName() : "");
        if (r.getMatchedText() != null) {
            h.tvMatch.setText(r.getMatchedText());
            h.tvMatch.setVisibility(View.VISIBLE);
        } else {
            h.tvMatch.setVisibility(View.GONE);
        }
        if (r.getTranscript() != null) {
            h.tvTime.setText("时间: " + String.format("%02d:%02d",
                    r.getTranscript().getSegmentStart() / 60, r.getTranscript().getSegmentStart() % 60));
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        CardView card;
        TextView tvType, tvTitle, tvStation, tvMatch, tvTime;
