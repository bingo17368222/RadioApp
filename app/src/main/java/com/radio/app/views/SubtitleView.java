...
public class SubtitleView extends LinearLayout {
    ...
    private View createItem(Transcript t, int idx) {
        ...
        card.setCardBackgroundColor(ContextCompat.getColor(getContext(), R.color.card_dark));
        card.setRadius(8);
        card.setContentPadding(16, 12, 16, 12);
        card.setTag(idx);
        LinearLayout layout = new LinearLayout(getContext());
        layout.setOrientation(VERTICAL);
        TextView tvTime = new TextView(getContext()); tvTime.setText(fmtTime(t.getSegmentStart())); tvTime.setTextColor(ContextCompat.getColor(getContext(), R.color.text_secondary)); tvTime.setTextSize(12);
        TextView tvText = new TextView(getContext()); tvText.setText(t.getText()); tvText.setTextColor(ContextCompat.getColor(getContext(), R.color.text_primary)); tvText.setTextSize(14); tvText.setPadding(0, 4, 0, 0);
    ...
    public void highlightSubtitle(long ms) {
        ...
        if (highlightIdx >= 0 && highlightIdx < container.getChildCount()) ((CardView) container.getChildAt(highlightIdx)).setCardBackgroundColor(ContextCompat.getColor(getContext(), R.color.card_dark));
        if (idx >= 0 && idx < container.getChildCount()) { ((CardView) container.getChildAt(idx)).setCardBackgroundColor(ContextCompat.getColor(getContext(), R.color.primary_dark)); scrollView.post(() -> scrollView.smoothScrollTo(0, container.getChildAt(idx).getTop())); }
        highlightIdx = idx;
    }
