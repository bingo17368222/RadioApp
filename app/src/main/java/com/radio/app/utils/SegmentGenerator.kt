...
object SegmentGenerator {
    ...
    private fun cutSongByLoudnessJump(segments: MutableList<VoiceSegment>, pcmFile: File?): Int {
        if (pcmFile == null || !pcmFile.exists() || segments.size <= 1) return 0
        segments.sortBy { it.start }
        var cutCount = 0
        var i = segments.size - 1
        while (i >= 0) {
            ...
            var added = false
            for ((a, b) in songRuns.sortedBy { it.first }) {
                val newPieces = mutableListOf<VoiceSegment>()
                for (p in pieces) {
                    if (p.hasVoice && a >= p.start && b <= p.end && (b - a) >= LOUD_MIN_RUN_MS) {
                        if (a > p.start) newPieces.add(p.copy(start = p.start, end = a, hasVoice = true, label = p.label))
                        newPieces.add(VoiceSegment(start = a, end = b, hasVoice = false, label = "歌曲", isSimulated = true))
                        if (b < p.end) newPieces.add(p.copy(start = b, end = p.end, hasVoice = true, label = p.label))
                        added = true
                    } else {
                        newPieces.add(p)
                    }
            ...
            if (added) {
                segments.removeAt(i)
                segments.addAll(i, pieces)
                cutCount += pieces.count { !it.hasVoice }
                i += pieces.size - 1
            }
            i--
        }
    ...
    /** 判定某标签是否为"干净可操作的干货/水标签"（非"分类失败"、非"歌曲"、非null）。 */
    private fun isCleanLabel(label: String?): Boolean {
        return label != null && label != "分类失败" && label != "歌曲"
    }
    ...
    /** 把窗索引换算成毫秒。startMs为段起点；end窗额外加上一个窗长以覆盖整窗。 */
    private fun winToMs(startMs: Long, winIdx: Int, winLenS: Double = 0.0): Long {
        return startMs + ((winIdx * LOUD_HOP_S + winLenS) * 1000).toLong()
    }
     ...
     * 规则（v3.1.232 收紧误判面，护歌优先；v3.1.233 不再依赖 isSimulated）：
     * - 段 must be 干 && 时长<=PCMS_TINY_DRY_MS
    ...
    private fun reabsorbTinyIsolatedSpeech(segments: MutableList<VoiceSegment>, pcmFile: File?): Int {
        ...
        while (i < segments.size - 1) {
            ...
            if (isolated) {
                ...
                // 水-干-水（以及嵌套的水-干-水-干-水…）全并回一个连续水段。
                // 修复 v3.1.231/232 只扩到 dry.end 导致"两个连续水分段未合并"的问题。
                val mergedWater = left.copy(end = maxOf(cur.end, right.end))
                segments.removeAt(i)   // remove cur
                segments.removeAt(i)   // now i points to right (was i+1)
                segments[i - 1] = mergedWater
                reabsorbed++
                // 不递增：新的 mergedWater 可能与下一个干段再次构成 水-干-水 链式结构，
                // 使整首被误切的歌一次全部还原为一个连续水段。
            } else {
                i++
            }
        }
        // 最后再合并一次相邻水段（把还残留的相邻水段连起来，消除"连续水分段"）
        mergeAdjacentWaterSegments(segments)
        return reabsorbed
    }

    /** v3.1.233-fix: 合并相邻的连续水段为一段（消除"两个连续水分段未合并"）。 */
    private fun mergeAdjacentWaterSegments(segments: MutableList<VoiceSegment>) {
        if (segments.size <= 1) return
        var i = 0
        while (i < segments.size - 1) {
            val a = segments[i]
            val b = segments[i + 1]
            if (!a.hasVoice && !b.hasVoice) {
                segments.removeAt(i + 1)
                segments[i] = a.copy(end = b.end)
                // 不递增，继续与下一个水段合并
            } else {
                i++
            }
    ...
    fun generateJiuAiTingSegments(
        ...
        // v3.1.230-fix: 先还原孤立于长水段中的极短干段为水，护住歌曲完整性。
